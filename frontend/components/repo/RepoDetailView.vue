...
function buildRepoPath(path: string) {
  const base = `/${encodeURIComponent(String(route.params.username || ''))}/${encodeURIComponent(String(route.params.slug || ''))}`
  const normalized = normalizeRoutePath(path)
  return normalized ? `${base}/${encodePathForUrl(normalized)}` : base
}

const routeDirectoryPath = computed(() => normalizeRoutePath(route.params.path))
const legacyQueryPath = computed(() => normalizeRoutePath(route.query.path))
const currentPath = ref(routeDirectoryPath.value || legacyQueryPath.value)
...
async function navigateToPath(path: string) {
  ...
  try {
    ...
    const nextQuery = Object.fromEntries(Object.entries(route.query).filter(([key]) => key !== 'path'))
    await router.push({
      path: buildRepoPath(normalized),
      query: nextQuery,
    })
  } finally {
...
watch(
  [routeDirectoryPath, legacyQueryPath],
  ([directoryPath, queryPath]) => {
    const normalized = directoryPath || queryPath
...
async function saveRepoDetailsEdit() {
  ...
  try {
    ...
    showRepoEditModal.value = false
    repo.value = updated
    const expectedPath = buildRepoPath(currentPath.value)
